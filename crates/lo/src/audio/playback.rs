...
//!
//! Raw mono PCM (e.g. Kokoro 24 kHz) is resampled to the device output rate and
//! scheduled back-to-back for gapless speech, while a spectrum tee feeds the
//! "speaking" orb.
//!
//! Gaplessness falls out of a single ring buffer: the output callback pulls
//! from one ring at the device rate and writes silence whenever the ring runs
//! dry, so queued chunks play seamlessly and underruns are silent rather than
//! glitchy.
//!
