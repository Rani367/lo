...
//! active OpenAI-compatible engine. It owns the request/stream half; the
//! `MAX_ROUNDS` agent loop itself lives in the orchestrator (the worker), which
//! calls [`stream_completion`] once per round.
//!
//! All of the *parsing* (turning raw `data:` lines into prose + a `Vec<ToolCall>`)
//! lives in `lo_core::brain::sse::StreamAccumulator`; this module only owns the
//! transport: POST the body, stream `bytes_stream()`, split on `\n`, and feed each
//! line into the accumulator until the terminal `[DONE]`.
...
///
/// Forwards each assistant prose delta to `on_delta` as it arrives and accumulates
/// any native `tool_calls`. Returns the final
    ...
    let res = {
        ...
        loop {
            ...
            if let Some(key) = endpoint.api_key.as_deref() {
                ...
                // local servers stay unauthenticated.
            ...
            match req.send().await {
                ...
                Ok(res) => {
                    ...
                    // Surface the status with a truncated body (first 300 chars).
