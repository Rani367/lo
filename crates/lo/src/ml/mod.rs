//! On-device ML subsystem: speech-to-text, text-to-speech, and voice-activity
//! detection. Everything runs locally — no cloud calls, no API keys.
//!
//! Each engine is **feature-gated** so a host without a C/C++/CMake toolchain (or
//! building offline) still produces a working binary:
...
//! type names still resolve, so callers compile unchanged.
//!
