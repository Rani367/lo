...
//!
//! The lifecycle semantics are deliberate: intentional-stop bookkeeping (a
//! deliberate kill is not a crash), `Address already in use` detection on stderr,
//! and restart-rather-than-double-spawn so two instances never clash on the port.
//!
...
//! background reaper, a poll-based lifecycle that avoids a long-lived task that
//! would pin the handle.
...
/// The lifecycle state of a managed server.
